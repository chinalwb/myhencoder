...
public class VerticalNumberChangerView_0 extends View {
    ...
    public VerticalNumberChangerView_0(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    private void fixOffsetY(float offset) {
        if (offset > 0) {
            offsetY = offset + (-MAX_HEIGHT + getMeasuredHeight());
        } else if (offset < -MAX_HEIGHT + getMeasuredHeight()) {
            offsetY = offset - (-MAX_HEIGHT + getMeasuredHeight());
        }
...
//        Log.e("XX", "offset == " + offset + ", modResult == " + modResult + ", UNIT_HEIGHT / 2F == " + (UNIT_HEIGHT / 2f));
